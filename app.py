...
def sign_up():
    ...
    doc = {
        ...
        "profile_name": username_receive,  # nama pengguna disetel ke id mereka secara default
        "profile_pic": "",  # nama file gambar profil
        "profile_pic_real": "profile_pics/profile_placeholder.png",  # gambar profil default
        "profile_info": "",  # deskripsi
    }
...
def check_dup():
    # ID kita harus memeriksa apakah id tersebut sudah diambil atau belum
    username_receive = request.form["username_give"]
...
def update_like():
    ...
    try:
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=["HS256"])
        # mengubah jumlah suka untuk postingan
        user_info = db.users.find_one({"username": payload["id"]})
